...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ---- 4. INPUT MANUAL (v2.0: berbasis periode cut-off, jml_tm dihitung otomatis) ----
    elseif (isset($_POST['submit_manual'])) {
        require_once __DIR__ . '/../../includes/honor_helper.php';

        $m_bulan        = $_POST['manual_bulan']        ?? '';
        $m_tahun        = (int)($_POST['manual_tahun']  ?? date('Y'));
        $m_jadwal       = $_POST['manual_jadwal']       ?? '';
        $m_cutoff       = max(1, (int)($_POST['manual_cutoff'] ?? 25));
        $m_min_tm       = max(1, (int)($_POST['manual_min_tm'] ?? 3));
        ...
        if (empty($m_bulan)) {
            ...
            $qJdwl = mysqli_query($koneksi, "SELECT j.id_jdwl, j.semester, COALESCE(u.honor_persks,50000) as honor_persks
                FROM t_jadwal j LEFT JOIN t_user u ON j.id_user=u.id_user WHERE j.id_jdwl = '$id_jadwal'");
            if (mysqli_num_rows($qJdwl) == 0) {
                $error_message = "❌ Jadwal dengan ID $id_jadwal tidak ditemukan!";
            } else {
                $rowJdwl    = mysqli_fetch_assoc($qJdwl);
                $honor_persks = (int)$rowJdwl['honor_persks'];

                // Hitung periode cut-off
                $periode = getPeriodeForBulan($m_bulan, $m_tahun, $m_cutoff);

                // Proses menggunakan business logic baru
                $hasil = prosesHonorPeriode(
                    $koneksi,
                    $id_jadwal,
                    $periode['periode_awal'],
                ...
                if ($hasil['status'] === 'pending') {
                    $success_message = "✅ Data honor berhasil diproses! " . $hasil['message'];
                } elseif ($hasil['status'] === 'digabung') {
                    $success_message = "⚠️ Pertemuan belum cukup. " . $hasil['message'];
                } else {
                    $error_message = "❌ " . ($hasil['message'] ?? 'Gagal memproses');
                }
            }
        }
    }
...
$q_recent = mysqli_query($koneksi,
    "SELECT h.id_thd, h.bulan, h.jml_tm, h.sks_tempuh,
            h.periode_awal, h.periode_akhir, h.status, h.total_honor,
            j.id_jdwl, j.kode_matkul, j.nama_matkul,
                ...
                <div class="up-card-body">
                    ...
                    <div class="up-recent">
                        ...
                        <form method="POST" id="formManual">
                            ...
                            <!-- Info periode cut-off otomatis -->
                            <div id="infoSemester" class="alert alert-info py-2 small mb-3" style="border-left: 4px solid var(--info);">
                                <i class="fas fa-calendar-check mr-1"></i>
                                <strong>Sistem v2.0:</strong> Jumlah TM dan SKS dihitung <em>otomatis</em> dari data pertemuan
                                yang sudah diinput di halaman <a href="<?= BASE_URL ?>koordinator/upload_data/pertemuan_dosen.php">Input Pertemuan</a>.
                                Tidak perlu input manual jumlah TM.
                            </div>

                            <div class="up-form-grid" style="grid-template-columns: repeat(2, 1fr);">
                                <div class="up-form-group">
                                    <label class="up-form-label">Tahun</label>
                                    <input type="number" class="up-input" name="manual_tahun" value="<?= date('Y') ?>" min="2020" max="2030">
                                    <span class="up-form-hint">Tahun periode honor</span>
                                </div>
                                <div class="up-form-group">
                                    <label class="up-form-label">Minimum Pertemuan</label>
                                    <input type="number" class="up-input" name="manual_min_tm" value="3" min="1" max="20">
                                    <span class="up-form-hint">Default: 3 (< ini → digabung)</span>
                                </div>
                            </div>
                            ...
                            <table class="up-table" id="tableHonor">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($recent_data as $no => $row):
                                        $statusMap = ['pending'=>['warning','clock'],'digabung'=>['info','layer-group'],'dibayar'=>['success','check-circle']];
                                        $st = $statusMap[$row['status']??'pending'] ?? ['secondary','question'];
                                    ?>
                                    <tr>
                                        ...
                                        <td><strong><?= safe_html($row['kode_matkul'] ?? '') ?></strong></td>
                                        ...
                                        <td class="text-right"><strong><?= 'Rp '.number_format((int)($row['total_honor']??0),0,',','.') ?></strong></td>
                                        <td class="text-center">
                                            <span class="badge badge-<?= $st[0] ?>">
                                                <i class="fas fa-<?= $st[1] ?> mr-1"></i><?= ucfirst($row['status']??'pending') ?>
                                            </span>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
